...
router.get("/shape", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const [k, v] of Object.entries(req.query)) {
        if (v == null)
            continue;
        if (!pass.has(k) && !k.startsWith("subset__"))
            continue;
        if (Array.isArray(v))
    ...
    const upstream = yield fetch(url, {
        ...
        headers: {
            authorization: (_a = req.headers.authorization) !== null && _a !== void 0 ? _a : "",
    ...
    res.setHeader("Access-Control-Expose-Headers", "electric-offset,electric-handle,electric-schema,electric-cursor,next-offset");
    if (!upstream.body)
        return res.end();
    node_stream_1.Readable.fromWeb(upstream.body).pipe(res);
}));
