...
const errorMiddleware = (err, req, res, _next) => {
    ...
    if (err instanceof client_1.Prisma.PrismaClientValidationError) {
        ...
        if (errors.length > 0) {
            ...
            requestLogger.error(`Prisma validation error: ${errorMessage}`, {
                source: "prisma",
                code: "VALIDATION_ERROR",
                statusCode: https_status_codes_1.default.BAD_REQUEST,
                stack: err.stack,
                errorName: err.name,
                details: { errors },
            });
    ...
    if (err instanceof client_1.Prisma.PrismaClientInitializationError) {
        ...
        requestLogger.error(`Database connection error: ${err.message}`, {
            source: "prisma",
            code: "DB_CONNECTION_ERROR",
            statusCode: https_status_codes_1.default.SERVICE_UNAVAILABLE,
            stack: err.stack,
            errorName: err.name,
    ...
    if (err instanceof client_1.Prisma.PrismaClientRustPanicError) {
        ...
        requestLogger.error(`Critical database error: ${err.message}`, {
            source: "prisma",
            code: "DB_PANIC_ERROR",
            statusCode: https_status_codes_1.default.INTERNAL_SERVER_ERROR,
            stack: err.stack,
            errorName: err.name,
        });
