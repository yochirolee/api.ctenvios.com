        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (process.env.NODE_ENV === "development") {
                    console.warn("Config cache refresh failed (table may not exist):", error.message);
                }
...
setImmediate(() => {
    exports.configService.refreshCache().catch((error) => {
        // Silently fail - cache will be populated on first use
        if (process.env.NODE_ENV === "development") {
            console.warn("Config cache initialization deferred (table may not exist yet):", error.message);
        }
    });
});
