...
const analytics = {
    ...
    getPackagesAndWeightInAgencies: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        var _a, _b;
        const user = req.user;
        const { status: statusQuery } = req.query;
        const isAdmin = isAdminUser(user.role);
        let agencyId = (_a = user.agency_id) !== null && _a !== void 0 ? _a : undefined;
        if (!isAdmin && user.agency_id) {
            const agency = yield repositories_1.repository.agencies.getById(user.agency_id);
            if ((agency === null || agency === void 0 ? void 0 : agency.agency_type) === client_1.AgencyType.FORWARDER)
                agencyId = undefined;
        }
        else if (isAdmin) {
            agencyId = undefined;
        ...
        let statusIn;
        if (statusQuery) {
            const parts = (typeof statusQuery === "string" ? statusQuery : String((_b = statusQuery[0]) !== null && _b !== void 0 ? _b : ""))
                .split(",")
                .map((s) => s.trim())
                .filter(Boolean);
            const parsed = parts.filter((s) => validStatuses.includes(s));
        ...
        const packages = yield repositories_1.repository.analytics.getPackagesAndWeightInAgencies(agencyId, statusIn);
