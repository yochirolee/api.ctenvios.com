...
exports.clearInvoiceCaches = clearInvoiceCaches;
...
function calculateInvoiceTotals(order) {
    ...
    const chargeAmount = (0, utils_1.formatCents)(order.charge_in_cents || 0);
...
const generateInvoicePDF = (invoice) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const doc = new pdfkit_1.default({ margin: 10, size: "letter" });
        registerCustomFonts(doc);
        yield generateOptimizedInvoice(doc, invoice);
        return doc;
    }
    catch (error) {
...
function generateOptimizedInvoice(doc, invoice) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        // Pre-load assets - convert Cloudinary URLs to PNG (PDFKit doesn't support WEBP)
        const agencyLogoUrl = convertCloudinaryToPng((_a = invoice.agency.logo) !== null && _a !== void 0 ? _a : undefined);
        const [logoBuffer, barcodeBuffer] = yield Promise.all([loadLogo(agencyLogoUrl), generateBarcode(invoice.id)]);
        ...
        yield generatePageHeader(doc, invoice, logoBuffer, barcodeBuffer, formattedData, calculations, true);
        ...
        const result = yield generateItemsTableOptimized(doc, invoice, calculations, logoBuffer, barcodeBuffer, formattedData);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (logoUrl.startsWith("http://") || logoUrl.startsWith("https://")) {
                // Load from URL
                const response = yield fetch(logoUrl);
                if (!response.ok) {
...
function generateBarcode(invoiceId) {
    return __awaiter(this, void 0, void 0, function* () {
...
function formatInvoiceData(invoice) {
    var _a, _b;
    const senderName = (0, capitalize_1.formatName)(invoice.customer.first_name, invoice.customer.middle_name, invoice.customer.last_name, invoice.customer.second_last_name, 30);
    const recipientName = (0, capitalize_1.formatName)(invoice.receiver.first_name, invoice.receiver.middle_name, invoice.receiver.last_name, invoice.receiver.second_last_name, 30);
    ...
    const location = `${((_a = invoice.receiver.city) === null || _a === void 0 ? void 0 : _a.name) || ""} ${((_b = invoice.receiver.province) === null || _b === void 0 ? void 0 : _b.name) || ""}`.trim();
...
function generatePageHeader(doc_1, invoice_1, logoBuffer_1, barcodeBuffer_1, formattedData_1, calculations_1) {
    return __awaiter(this, arguments, void 0, function* (doc, invoice, logoBuffer, barcodeBuffer, formattedData, calculations, isFirstPage = false) {
        let currentY = 10;
...
function generateSenderRecipientInfo(doc, invoice, formattedData) {
    const textStyle = new TextStyler(doc);
    let currentY = 115;
    ...
    if (invoice.receiver.mobile || invoice.receiver.phone) {
        const phoneText = `Tel: ${invoice.receiver.mobile || ""}${invoice.receiver.mobile && invoice.receiver.phone ? " - " : ""}${invoice.receiver.phone || ""}`;
        textStyle
            .style(FONTS.NORMAL, 9, COLORS.DARK_GRAY)
...
function generateItemsTableOptimized(doc, invoice, calculations, logoBuffer, barcodeBuffer, formattedData) {
    return __awaiter(this, void 0, void 0, function* () {
        let currentY = 200;
        let currentPage = 1;
        const textStyle = new TextStyler(doc);
        ...
        const processedItems = invoice.order_items.map((item, index) => {
            const weight = (0, utils_1.toNumber)(item.weight); // weight is Decimal, needs conversion
            return Object.assign(Object.assign({}, item), { hbl: item.hbl || `CTE${String(invoice.id).padStart(6, "0")}${String(index + 1).padStart(6, "0")}`, subtotal_in_cents: (0, utils_1.calculate_row_subtotal)(item.price_in_cents, // Int - already a number
                weight, item.customs_fee_in_cents, // Int - already a number
                item.charge_fee_in_cents || 0, // Int? - already a number
                item.insurance_fee_in_cents || 0, // Int? - already a number
                item.unit) });
        });
        const addNewPageWithHeaderFooter = () => __awaiter(this, void 0, void 0, function* () {
            doc.addPage();
            currentPage++;
            // Add header to new page
            yield generatePageHeader(doc, invoice, logoBuffer, barcodeBuffer, formattedData, calculations, false);
        ...
        for (let i = 0; i < processedItems.length; i += BATCH_SIZE) {
            ...
            for (const item of batch) {
                ...
                if (currentY + rowHeight > LAYOUT.PAGE_HEIGHT - LAYOUT.BOTTOM_MARGIN) {
                    currentY = yield addNewPageWithHeaderFooter();
                }
...
function addOptimizedTableHeaders(doc, y, textStyle) {
    const headers = [
...
function renderTableRow(doc, item, currentY, rowHeight, textStyle) {
    ...
    const rowData = [
        ...
        {
            text: `${(0, utils_1.formatCents)(item.insurance_fee_in_cents || 0)}`,
            x: 300,
            y: verticalCenter,
            width: 40,
            align: "right",
        ...
        {
            text: `${(0, utils_1.formatCents)(item.charge_fee_in_cents || 0)}`,
            x: 340,
            y: verticalCenter,
            width: 40,
            align: "right",
        ...
        {
            text: `${(0, utils_1.formatCents)(item.customs_fee_in_cents || 0)}`,
            x: 385,
            y: verticalCenter,
            width: 40,
            align: "right",
        ...
        {
            text: `${(0, utils_1.formatCents)(item.price_in_cents || 0)}`,
            x: 430,
            y: verticalCenter,
            width: 40,
            align: "right",
        ...
        {
            text: `${(0, utils_1.formatCents)(item.subtotal_in_cents)}`,
            x: 520,
            y: verticalCenter,
            width: 40,
            align: "right",
...
function renderTotalsSection(doc, calculations, startY, textStyle) {
    let currentY = startY;
    const totals = [
...
function addFooterToPage(doc, order, currentPage, totalPages = 1) {
    const textStyle = new TextStyler(doc);
    // Tracking info
...
function updateAllPageNumbers(doc, totalPages) {
