...
exports.generateSalesReportPdf = exports.getSalesReportData = void 0;
 ...
 */
const getSalesReportData = (startDate, endDate, agencyId, userId) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    ...
    for (const order of ordersToday) {
        ...
        userData.orderRows.push({
            ...
            receiver_city: ((_a = order.receiver.city) === null || _a === void 0 ? void 0 : _a.name) || "",
    ...
    for (const [, debtData] of debtOrderMap) {
        ...
        userData.orderRows.push({
            ...
            customer_phone: order.customer.mobile || "",
            receiver_name: `${order.receiver.first_name} ${order.receiver.last_name}`,
            receiver_city: ((_b = order.receiver.city) === null || _b === void 0 ? void 0 : _b.name) || "",
            service_name: order.service.name,
            hbl_count: hblCount,
            weight_lbs: Math.round(weight * 100) / 100,
            ...
            paid_cents: paidToday,
            pending_cents: 0,
            payment_status: client_1.PaymentStatus.PAID, // This payment is complete
            payment_methods: paymentMethods,
    ...
    if (userId) {
        const filteredUser = yield prisma_client_1.default.user.findUnique({
            where: { id: userId },
            select: { id: true, name: true },
        });
        if (filteredUser) {
 ...
 */
const generateSalesReportPdf = (startDate, endDate, agencyId, userId) => __awaiter(void 0, void 0, void 0, function* () {
    const data = yield (0, exports.getSalesReportData)(startDate, endDate, agencyId, userId);
    return new Promise((resolve, reject) => {
        ...
        const boxY = y;
        // Box 1: Órdenes
        drawSummaryBox(doc, LAYOUT.MARGIN, boxY, boxWidth, boxHeight, "Órdenes Hoy", `${data.summary.total_orders}`, `${data.summary.total_hbls} HBLs`);
        // Box 2: Total Facturado
        drawSummaryBox(doc, LAYOUT.MARGIN + boxWidth + 10, boxY, boxWidth, boxHeight, "Facturado Hoy", formatCents(data.summary.total_billed_cents), `${data.summary.total_weight_lbs} lbs`);
        // Box 3: Deudas Cobradas
        drawSummaryBox(doc, LAYOUT.MARGIN + (boxWidth + 10) * 2, boxY, boxWidth, boxHeight, "Deudas Cobradas", formatCents(data.summary.debt_collections_cents), "De días anteriores");
        // Box 4: Total Recaudado
        drawSummaryBox(doc, LAYOUT.MARGIN + (boxWidth + 10) * 3, boxY, boxWidth, boxHeight, "Total Recaudado", formatCents(data.summary.total_collected_cents), data.summary.total_charges_cents > 0 ? `+${formatCents(data.summary.total_charges_cents)} cargos` : "");
        // Box 5: Pendiente
        drawSummaryBox(doc, LAYOUT.MARGIN + (boxWidth + 10) * 4, boxY, boxWidth, boxHeight, "Pendiente", formatCents(data.summary.total_pending_cents), "");
        y = boxY + boxHeight + 20;
        // ========== PAYMENT METHODS ==========
        if (data.paymentBreakdown.length > 0) {
            ...
            const methodTexts = data.paymentBreakdown.map((p) => `${PAYMENT_METHOD_LABELS[p.method]}: ${formatCents(p.total_amount_cents)} (${p.total_payments} pagos)`);
        ...
        for (const user of data.users) {
            ...
            doc.font(FONTS.REGULAR).fontSize(8).fillColor(COLORS.GRAY);
            const debtText = user.summary.debt_collections_cents > 0
                ? ` | Deudas: ${formatCents(user.summary.debt_collections_cents)}`
                : "";
            const userSummary = `${user.summary.total_orders} órdenes | Facturado: ${formatCents(user.summary.total_billed_cents)} | Recaudado: ${formatCents(user.summary.total_collected_cents)}${debtText} | Pendiente: ${formatCents(user.summary.total_pending_cents)}`;
            ...
            if (user.paymentBreakdown.length > 0) {
                doc.font(FONTS.REGULAR).fontSize(8).fillColor(COLORS.GRAY);
                const userMethods = user.paymentBreakdown.map((p) => `${PAYMENT_METHOD_LABELS[p.method]}: ${formatCents(p.amount_cents)}`);
            ...
            for (const order of user.orders) {
                ...
                xPos = LAYOUT.MARGIN;
                const paymentMethodsText = order.payment_methods.length > 0
                    ? order.payment_methods.map((m) => PAYMENT_METHOD_LABELS[m]).join(", ")
                    : "-";
                // For debt collections, show original order date
                const dateToShow = order.is_debt_collection && order.original_order_date ? order.original_order_date : order.created_at;
                const rowData = [
                    `#${order.order_id}`,
...
function drawSummaryBox(doc, x, y, width, height, label, value, subtitle) {
    // Border
...
exports.default = exports.generateSalesReportPdf;
