...
async function generateCleanCTEnviosLabel(
	...
	const barcodeText =
		item.hbl ||
			`REF${order.id.toString().padStart(8, "0")}${(itemIndex + 1).toString().padStart(3, "0")}`;

	// Large letter for transport type (left side)
	const transportLetter =
		order.service.service_type === ServiceType.AIR ? "A" :
		order.service.service_type === ServiceType.MARITIME ? "M" : "M";
	...
	doc
		...
		.font("Helvetica-Bold")
		.fillColor("#000000")
		.text(`${order.service.provider.name.toUpperCase()}`, margin + 140, currentY + 15, {
	...
	doc
		...
		.font("Helvetica")
		.fillColor("#000000")
		.text(order?.service?.forwarder.name || "", margin + 140, currentY + 34, {
			width: labelWidth - margin - 145,
			align: "right",
		});
...
async function generateProvinceLabel(
	...
	doc
		...
		.font("Helvetica-Bold")
		.text(
			`${order.receiver.province?.id || ""} - ${order.receiver.city?.id || ""}`,
			margin,
			labelHeight - 60,
			{
				width: labelWidth - margin * 2,
