...
export function invoiceHistoryMiddleware(userId: string, prisma: PrismaClient): Prisma.Middleware {
	return async (params, next) => {
		if (params.model === "Invoice" && params.action === "update") {
			const invoiceId = params.args.where.id;
			...
			const result = await next(params); // Ejecuta la actualización
			...
			for (const hbl of allHbls) {
				...
				if (prev && !curr) {
					...
					const itemFields = ["weight", "rate", "quantity", "description", "volume", "customs_fee"];
