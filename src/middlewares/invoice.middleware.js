            ...
            invoice: {
                update(_a) {
                    return __awaiter(this, arguments, void 0, function* ({ args, query }) {
                        ...
                        const previous = yield prisma.invoice.findUnique({
                            where: { id: invoiceId },
                            include: { items: true },
                        });
                        const result = yield query(args); // Ejecuta la actualización
                        // Obtener nuevo estado
                        const updated = yield prisma.invoice.findUnique({
                            where: { id: invoiceId },
                            include: { items: true },
                        });
                        if (!previous || !updated) {
                        ...
                        const prevItems = Object.fromEntries(previous.items.map((i) => [i.hbl, i]));
                        const newItems = Object.fromEntries(updated.items.map((i) => [i.hbl, i]));
