...
function validateAndSanitizeReceipt(receipt: ProcessedReceipt): ProcessedReceipt | null {
   ...
   if (sanitizedMobile) {
      ...
      if (sanitizedMobile.length < 10 || sanitizedMobile.length > 15) {
         console.warn(`⚠️ Invalid mobile format for CI ${sanitizedCi}, removing mobile: ${sanitizedMobile}`);
...
async function batchUpsertReceipts(receipts: ProcessedReceipt[]): Promise<void> {
   ...
   console.log(`📦 Processing ${receipts.length} receipts in ${chunks.length} batches of ${BATCH_SIZE}`);
...
async function batchUpsertReceiptsFast(receipts: ProcessedReceipt[]): Promise<void> {
   ...
   console.log(`🚀 Using fast batch processing: ${receipts.length} receipts in ${chunks.length} batches`);
      ...
      try {
         await prisma.$transaction(
            async (tx) => {
               ...
               console.log(
                  `   ✅ Batch ${i + 1}: ${newReceipts.length} created, ${existingReceiptsToUpdate.length} updated`
               );
            },
